...
| 名冊… | 在畫面右側打開／關閉名冊視窗 |
| 專注模式 | 30 分鐘／1 小時／2 小時／直到我取消：開會、簡報用。哥布林全部消失、**營地完全暫停**（也不計年齡）、不跳出通知也不出聲；選單列圖示變暗；時間到或按「結束專注模式」就回來 |
| 工作模式 | 同樣的時間選項：哥布林從畫面消失，但**營地在背景繼續運行**（出生、覓食、老死照常）；適合平常寫程式時常駐。時間到或按「結束工作模式」回來 |
...
選單「番茄鐘」：專注 25／休息 5、50／10、15／3 分鐘，或自訂。開始後有一隻哥布林（營地裡隨機一隻）走到主螢幕右上角，舉著電子時鐘倒數（`專注`：綠色 LCD，最後一分鐘變紅；時間到時哥布林跳起來、出聲提醒，時鐘轉成藍色的 `休息` 倒數；休息完哥布林說一句話後走開）。選單裡可看剩餘時間、隨時停止。聲音用「Claude 通知」的音量設定；專注模式或工作模式中只計時、不出聲，圖示旁會有通知數。實作在 `Pomodoro.swift`（計時與走位）與 `AntView.drawPomodoro`（七段顯示器）。
...
- **專注模式與工作模式中**：不跳出、不出聲，選單列圖示旁出現「● 數字」，結束模式的選單項會寫「期間有 N 則通知」。（工作模式下要不要照常跳出，見 SCENARIOS.md「待決定」。）
