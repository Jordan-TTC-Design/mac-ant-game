...
| 走動範圍與地圖 | （底部與左右兩邊的一條各有自己的場景：底部是河岸草地、池塘多；兩側是林緣、石頭與蘑菇多；營地沿著路排成一列，詳見 [TERRAIN.md](TERRAIN.md)）**整個螢幕**（預設）、**底部一條**（貼著 Dock 上緣）、**右邊一條**、**左邊一條**，或**獨立的營地視窗**。一條有多厚可選（薄、中〔預設，約 42 點高〕、厚、很厚），背景可選森林、草地或沒有；底部一條是橫向的樹林，左右兩邊的一條是直立的（樹一棵一棵往上疊，草地靠螢幕邊）；營地視窗有自己的草地，四周一圈森林，視窗拉多大都有，可以拖、縮放、縮到 Dock 或按關閉鈕收起來（不會自己彈回來）、回到右下角、設成永遠在最上面。背景會跟著電腦的時間變色：傍晚偏橘、夜晚偏藍 |
...
- **底部、右邊、左邊一條有了自己的場景**：底部是河岸草地（池塘多），兩側是林緣（石頭、蘑菇多）。有池塘、小溪與木橋、石頭、蘑菇、田、倒木，營地（帳篷、圖騰柱、曬肉架、火坑）沿著路排成一列，隨營地變大長出來，跟著季節變色，冬天路上有雪。釣魚、煮飯、耕田、砍倒木、採石都能在這幾種範圍裡做；木頭與石頭會慢慢再出現。
- **帳篷是入口**：帳篷和巢穴地洞在地下相通，哥布林會從最近的入口進去休息，也會從任何一個出來。
- **切換不再重來**：移動營地、切換走動範圍再切回來，地形不會重生；每個地方各自記得砍過什麼、種過什麼，營地也回到那個範圍原本的位置。換範圍時，正在做工作的哥布林不會再穿過整個螢幕跑去舊的地點。
- 修正：伴侶躺在床上被床墊蓋住、擁抱的手臂太細。
