...
| 池塘 | 營地視窗夠大（寬 ≥ 380、高 ≥ 280）時，左上方會有一座池塘。**形狀每次啟動都不一樣**：邊緣是不規則的（有灣、有岬），水從岸邊淺色到中間深色分幾層；岸上有沙土、幾顆石頭（有時一小堆）、蘆葦，水面有睡蓮、漣漪與閃光；大約一半的池塘中間有一座長著草叢的**小島**。哥布林會繞過水面與小島（依真正的形狀，不是方框）。拉大縮小視窗只是把同一個形狀伸縮，不會重新生成 |
...
| `CAMP_POND_SEED=數字` | 固定池塘的形狀（不設就是隨機） |
