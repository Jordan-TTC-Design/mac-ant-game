...
final class ManualWindow: NSObject {
    ...
    static let sections: [(title: String, lines: [String])] = [
        ...
        ("四種狀態與快捷鍵", [
            ...
            "• 可以在選單「哥布林出現在哪個桌面」選「所有桌面」，或勾選要出現的桌面。",
            "• 全螢幕 App（影片、簡報）自己的桌面永遠不會有哥布林。",
            "## 走動範圍與背景",
            "• 選單「走動範圍與背景」：整個螢幕（預設）、底部一條、右邊一條、左邊一條，或獨立的營地視窗。",
            ...
            "• 可以在選單「哥布林出現在哪個螢幕」改成「所有螢幕」，或勾選指定的螢幕。家會跟著營地走：把營地拖到另一個螢幕，家就換過去。",
        ...
        ("哥布林與公主", [
            ...
            "• 名冊：選單「哥布林名冊…」在畫面右側列出每一隻，點一隻，畫面上會圈出牠並顯示名字。",
        ...
        ("營地、食物與自然事件", [
            ...
            "• 「魔獸來襲」：全開模式下，偶爾會有史萊姆、巨鼠從螢幕邊走進來。哥布林會出來圍打，受傷的會回營地休息；公主會躲進洞裡，之後幫傷兵治療。魔獸被打倒會掉素材，哥布林搬回營地，可以在「倉庫與魔獸圖鑑…」查看。可以在選單調整頻率或關閉。",
            "• 哥布林閒著會做自己的事：睡覺（晚上最愛）、去池塘釣魚、玩耍、看書、鬧著玩打一架（不會受傷）。聰明的不打架，愛看書；金皮會戴著小金冠巡視，普通哥布林在旁邊伺候。名冊點一隻可以看到牠現在在做什麼。",
            ...
            "• 營地視窗是一個場景，每個營地都不一樣：草地、森林空地、雪地或沼澤，有大小不同的池塘、小溪與木橋、樹叢、石頭、小路。哥布林住得很原始：巢穴四周是泥地，有獸皮帳篷、圖騰柱、曬肉架和石頭圍起來的營火坑。哥布林會繞過水、樹和石頭。選單「走動範圍 → 營地視窗的地貌」可以換主題或重新生成。營地是慢慢長大的：一開始只有巢穴，哥布林變多才會有火坑、帳篷、曬肉架、圖騰柱。",
        ...
        ("常見問題", [
            ...
            "• 預設只有「桌面 1」有哥布林；在別的桌面請到選單「哥布林出現在哪個桌面」勾選。",
            "• 有多個螢幕時，哥布林預設只在營地所在的螢幕；其他螢幕請到選單「哥布林出現在哪個螢幕」勾選。",
