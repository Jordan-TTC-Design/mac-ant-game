...
final class ManualWindow: NSObject {
    ...
    static let sections: [(title: String, lines: [String])] = [
        ...
        ("資料、更新與移除", [
            ...
            "• 更新：哥布林營地每天會自己看一次有沒有新版本，有的話跳出來問你。按「更新」就會自己下載、換好、重新打開，進度不會不見，Claude Code 的連接也不用重做。",
            "• 想自己看的話，選單裡有「檢查更新…」；不想被問就把「偏好設定 → 自動檢查更新」的勾拿掉。",
            "• 這是 App 唯一會用到網路的地方，只去讀一個版本號檔案，不會傳送任何你的資料。",
            "• 如果自動更新失敗（例如 App 放在不能寫入的地方），就先從選單「結束哥布林營地」，再用新的 GoblinCamp.app 取代舊的（放在同一個位置）。",
